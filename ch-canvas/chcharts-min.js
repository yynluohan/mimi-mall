...
function initChart(v1, wcPNAjO2, V3) {
    console.log('chcharts-min.js-----initChart---this["\x6f\x70\x74\x69\x6f\x6e\x73"]["\x73\x74\x79\x6c\x65"]',this["\x6f\x70\x74\x69\x6f\x6e\x73"]["\x73\x74\x79\x6c\x65"])
  switch (this["\x6f\x70\x74\x69\x6f\x6e\x73"]["\x73\x74\x79\x6c\x65"]) {
    case "\x70\x69\x65": {
    console.log('chcharts-min.js-----initChart---A---\x70\x69\x65',"\x70\x69\x65")
      A(this, v1, wcPNAjO2, V3);
      break;
    }
    case "\x72\x6f\x73\x65": {
    console.log('chcharts-min.js-----initChart---B---\x70\x69\x65',"\x70\x69\x65")

      B(this, v1, wcPNAjO2, V3);
      break;
    }
    case "\x6c\x69\x6e\x65": {
    console.log('chcharts-min.js-----initChart---C---\x70\x69\x65',"\x70\x69\x65")

      C(this, v1, wcPNAjO2, V3);
      break;
    }
    case "\x72\x69\x6e\x67": {
    console.log('chcharts-min.js-----initChart---D---\x70\x69\x65',"\x70\x69\x65")

      D(this, v1, wcPNAjO2, V3);
      break;
    }
    case "\x72\x61\x64\x61\x72": {
    console.log('chcharts-min.js-----initChart---E---\x70\x69\x65',"\x72\x61\x64\x61\x72")

      E(this, v1, wcPNAjO2, V3);
      break;
    }
    case "\x62\x75\x62\x62\x6c\x65": {
    console.log('chcharts-min.js-----initChart---F---\x70\x69\x65',"\x70\x69\x65")

      F(this, v1, wcPNAjO2, V3);
      break;
    }
    case "\x62\x61\x72": {
    console.log('chcharts-min.js-----initChart---G---\x70\x69\x65',"\x70\x69\x65")
  ...
  if (rectStyle == "accum") {
    function Gs(valueArray, color, length, xDesc, yDesc) {
      ...
      for (let i = 0; i < length; i++) {
        curY =
          ((valueArray[i] - yDesc.bottom) / yDesc.range) *
          (barheight - pdTop - pdBottom);
        X1 = curX;
        y1 = curY;
        curX += xDesc.span;
    ...
    let xAxis = new XAxis({
      ...
      context: context,
      dataArray: xLabel,
      optional: { rowWordsCnt: 3, startPadding: 50, endPadding: 50 }
    });
    xAxis.draw();
...
function E(that, context, width, height) {
  ...
  var ratio = 0.8;
  if (that.options.chartRatio > 0 && that.options.chartRatio <= 1)
    ratio = that.options.chartRatio;
  ...
  var colorArr = ['#f5c8b4', '#e2ae97','#cd937a']
  var animation = function() {
    if (step <= MaxStep) {
      for (var layer = 3; layer > 0; layer--) {
        ...
        context.setFillStyle(colorArr[layer-1]);
        ...
        // }
        var currentRad = (layer / 3) * radius;
        context.moveTo(center.x, center.y - currentRad);
        var currentAngle = -Math.PI / 2;
        for (var i = 0; i < length; i++) {
          context.lineTo(
            center.x + currentRad * Math.cos(currentAngle),
            center.y + currentRad * Math.sin(currentAngle)
          );
          currentAngle += angle;
        }
        context.fill();
      ...
      context.closePath();
		// var MaxValue = Math.max.call(null, that.options.data[0].value);
      ...
      that.options.data.forEach(function(val) {
        ...
        var currentRad =
          (((radius * val.value[0]) / MaxValue) * step) / MaxStep;
        context.moveTo(center.x, center.y - currentRad);
        var currentAngle = -Math.PI / 2;
        for (var i = 0; i < length; i++) {
    ...
    if (step <= MaxStep) {
      ...
      that.options.data.forEach(function(val, idx) {
        ...
        context.arc(
          center.x,
          center.y,
          (((radius * val.value) / MaxStep) * step) / maxValue,
          start,
...
function A(that, context, width, height) {
  ...
  var small = width > height ? height : width;
  var radius = (small * ratio) / 2.0;
  var pieCenter = { x: width / 2.0, y: height / 2.0 };
  var total = 0;
  that.options.data.forEach(function(val) {
    total += val.value;
  });
...
function Axis(param) {
  ...
  this.rowWordsCnt = param.optional.rowWordsCnt
    ? param.optional.rowWordsCnt
    : this.orientation == "horizontal"
    ? 2
    : parseInt(
        (this.width - this.lineWidth / 2 - this.textAxisMargin) / this.fontSize
      ) || 1;
  ...
  this.textWidth =
    this.orientation == "horizontal"
      ? this.fontSize * this.rowWordsCnt
      : this.width - this.lineWidth / 2 - this.textAxisMargin;
  this.rowCnt = param.optional.rowCnt
    ? param.optional.rowCnt
    : this.orientation == "horizontal"
    ? parseInt(
        (this.height -
          this.lineWidth / 2 -
          this.textAxisMargin +
          this.rowMargin) /
          (this.fontSize + this.rowMargin)
      ) || 1
    : 1;
  this.textHeight =
    this.orientation == "horizontal"
      ? this.height - this.lineWidth / 2 - this.textAxisMargin
      : (this.fontSize + this.rowMargin) * this.rowCnt - this.rowMargin;
}
Axis.prototype.getDescriptor = function() {
  ...
  if (this.dataType == "number") {
    ...
    const cnt =
      this.orientation == "horizontal"
        ? parseInt(totalLength / (this.textWidth + this.textMargin)) || 1
        : parseInt(totalLength / (this.textHeight + this.textMargin)) || 1;
    let mStep =
      min >= 0 ? max / cnt : max <= 0 ? -min / cnt : (max - min) / cnt;
    ...
    if (
      ...
      desc.step =
        this.orientation == "horizontal"
          ? Math.ceil((this.textWidth + this.textMargin) / desc.span)
          : Math.ceil((this.textHeight + this.textMargin) / desc.span);
      desc.startPosOffset = this.startPadding;
    }
  }
...
Axis.prototype.draw = function() {
  ...
  if (this.dataType == "string") {
    ...
    let curPos =
      this.orientation == "horizontal"
        ? x0 + desc.startPosOffset
        : y0 - desc.startPosOffset;
    for (let i = 0; i * desc.step < this.dataArray.length; i++) {
      ...
      for (let j = 0; j < rowNum; j++) {
        ...
        let text = "";
        if (j == rowNum - 1 && this.rowCnt < itNum)
          text = name.substring(sPos, ePos - 1) + "…";
        else text = name.substring(sPos, ePos);
        if (this.orientation == "horizontal") {
          context.fillText(
            text,
            curPos,
            y0 + this.textAxisMargin + j * (this.fontSize + this.rowMargin)
          );
        } else {
          context.fillText(
            text,
            x0 - this.textAxisMargin,
            curPos +
              j * (this.fontSize + this.rowMargin) -
              ((rowNum - 1) * (this.fontSize + this.rowMargin)) / 2
          );
        }
        sPos = ePos;
      }
    ...
    if (desc) {
      for (let i = 0; i <= desc.cnt; i++) {
        ...
        if (this.orientation == "horizontal") {
          context.fillText(
            text,
            x0 + desc.startPosOffset + desc.space * i,
            y0 + this.textAxisMargin
          );
          context.moveTo(x0 + desc.startPosOffset + desc.space * i, y0);
          context.lineTo(
            x0 + desc.startPosOffset + desc.space * i,
            y0 - this.shortLineLength
          );
        } else {
          context.fillText(
            text,
            x0 - this.textAxisMargin,
            y0 - desc.startPosOffset - desc.space * i
          );
          context.moveTo(x0, y0 - desc.startPosOffset - desc.space * i);
          context.lineTo(
            x0 + this.shortLineLength,
            y0 - desc.startPosOffset - desc.space * i
          );
        }
        context.stroke();
...
function YAxis(param) {
  ...
  param.dataType == undefined
    ? param.dataArray instanceof Array && param.dataArray.length > 0
      ? (param.dataType = typeof param.dataArray[0])
      : null
    : null;
...
function C(that, context, _width, _height) {
  ...
  function Cs(valueArray, color, length, xDesc, yDesc) {
    ...
    for (let i = 0; i < length; i++) {
      let curY = yDesc
        ? (1 - (valueArray[i] - yDesc.bottom) / yDesc.range) *
            (height - pdTop - pdBottom) +
          pdTop
        : height - pdBottom;
      curY = curY || 0;
      if (i == 0) {
  ...
  let yAxis = new YAxis({
    ...
    context: context,
    dataArray: sumArr,
    optional: { lineWidth: lw, fontSize: fontSize }
  });
  yAxis.draw();
  let xAxis = new XAxis({
    height: pdBottom,
...
function F(that, context, _width, _height) {
  ...
  let xAxis = new XAxis({
    ...
    context: context,
    dataArray: xLabel,
    optional: { rowWordsCnt: 2, startPadding: padding, endPadding: padding }
  });
  xAxis.draw();
  let xDesc = xAxis.getDescriptor();
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (!points[i]) {
      ...
      for (
        var j = 0;
        j < Math.min.apply(null, [item.value.length, xLabel.length, y.length]);
        j++
      ) {
...
function GG(that, posX, posY, width, height) {
  ...
  else if (posY > height - barDesc.paddingBottom)
    top = height - barDesc.paddingBottom;
  ...
  else if (posX > width - barDesc.paddingRight)
    tranX =
      width -
      barDesc.paddingRight -
      barDesc.paddingLeft -
      barDesc.xDesc.startPosOffset;
  let dataIndex = Math.round(tranX / barDesc.xDesc.span);
  let left = parseInt(
    dataIndex * barDesc.xDesc.span +
      barDesc.paddingLeft +
      barDesc.xDesc.startPosOffset
  );
  toolTipInfo.tooltipPos =
    "left:10px;" + "right:0px;" + "top:10px;" + "font-size:14px";
...
function CC(that, posX, posY, width, height) {
  ...
  else if (posY > height - lineDesc.paddingBottom)
    top = height - lineDesc.paddingBottom;
  toolTipInfo.crossHLine =
    "width:" +
    (width - lineDesc.paddingLeft - lineDesc.paddingRight) +
    "px;" +
    "left:" +
    lineDesc.paddingLeft +
    "px;" +
    "top:" +
    top +
    "px;";
  ...
  if (posX < lineDesc.paddingLeft + lineDesc.xDesc.startPosOffset) tranX = 0;
    ...
    posX >
    width - lineDesc.paddingRight - (lineDesc.xDesc.endPadding || 0)
  )
    tranX =
      width -
      lineDesc.paddingRight -
      lineDesc.paddingLeft -
      lineDesc.xDesc.startPosOffset -
      (lineDesc.xDesc.endPadding || 0);
  let dataIndex = Math.round(tranX / lineDesc.xDesc.span);
  let left = parseInt(
    dataIndex * lineDesc.xDesc.span +
      lineDesc.paddingLeft +
      lineDesc.xDesc.startPosOffset
  );
  toolTipInfo.crossVLine =
    "height:" +
    (height - lineDesc.paddingBottom - lineDesc.paddingTop) +
    "px;" +
    "left:" +
    left +
    "px;" +
    ...
    "px;";
  toolTipInfo.tooltipPos =
    (left > width / 2 ? "left:0px;" : "right:0px;") +
    "top:0px;" +
    "font-size:14px";
  toolTipInfo.tooltipInfo = "";
  if (that.options && that.options.data) {
...
function FF(that, posX, posY, canvasWidth, canvasHeight) {
  if (
    that.options.showTooltip == undefined ||
    that.options.showTooltip == false
  ) {
    return {};
  }
  let ret = {};
  if (
    posY > bubble_coor.paddingTop &&
    posY < canvasHeight - bubble_coor.paddingBottom &&
    posX > bubble_coor.paddingLeft - 10 &&
    posX < canvasWidth + bubble_coor.paddingLeft
  ) {
    let clicked_point = {
    ...
    for (var i = 0, item, l = bubble_coor.points.length; i < l; i++) {
      ...
      for (var j = 0, obj, jl = item.data.length; j < jl; j++) {
        ...
        if (dis < Math.max(obj.radius, 4)) {
          ...
          ret.tooltipPos =
            (position.x > canvasWidth / 2 ? "left:0px;" : "right:0px;") +
            "top:0px;" +
            "font-size:14px";
          break;
        }
